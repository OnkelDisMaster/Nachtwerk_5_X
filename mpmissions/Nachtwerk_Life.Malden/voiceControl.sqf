...
if(hasInterface)then{'' spawn{
	...
	VoN_Event_fnc = {
		...
		if((VoN_channelId select 1) >= 0 && !((VoN_channelId select 1) in VC_allowedChannels))then{
			...
			cutText[format["Redem im %1 ist nicht erlaubt!\nÄndere deinen Kanal mit %2 oder %3", str localize(VoN_channelId select 0), actionKeysNames ["nextChannel",1], actionKeysNames ["prevChannel",1]], "PLAIN DOWN", 1];
			setCurrentChannel 5;
		};
	};
