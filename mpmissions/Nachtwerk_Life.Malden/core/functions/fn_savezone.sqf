...
private ["_eh1","_inArea","_zone1","_zone2","_dis"];
_zone1 = getMarkerPos "safezone_latrinite"; // MARKERS FOR SAFEZONE
_zone2 = getMarkerPos "reb_nord";
	...
	case west:
	{
		while {true} do
		{
			...
			{	
				...
				if (((_zone1 distance player > _dis) && (_zone2 distance player > _dis) ) && (_inArea)) then 
				{ 
					_inArea = false;
