...
if(_drinky > 0.4) then {
    [1,format["Alkoholtest %1 \n\n Promille: %2 \n\n Nicht im legalen Bereich! ",name player,[_drinky] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",_medic];
} else {
    [1,format["Alkoholtest %1 \n\n Promille: %2 \n\n Im legalen Bereich! ",name player,[_drinky] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",_medic];
};
...
if(_drugy > 0) then {
    [1,format["Drogentest %1 \n\n Ergebnis: Positiv \n\n Der Spieler steht unter Drogeneinfluss! ",name player]] remoteExec ["life_fnc_broadcast",_medic];
} else {
    [1,format["Drogentest %1 \n\n Ergebnis: Negativ \n\n Der Spieler ist clean! ",name player]] remoteExec ["life_fnc_broadcast",_medic];
};