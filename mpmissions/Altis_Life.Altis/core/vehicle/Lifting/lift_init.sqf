...
_objektTypen = (LIFE_SETTINGS(getArray,"life_container"));
...
//  if ((vehicle player isKindOf "Helicopter") && ((count (nearestObjects [vehicle player, ["Car", "Tank", "Ship"], 10]) > 0) || (count (nearestObjects [vehicle player, _objektTypen, 10]) > 0)) && (_speed < 10) && (driver vehicle player == player) && !(vehicle player in CUP_heloBlacklist) && !(typeOf vehicle player in CUP_heloBlacklist)) then {_return = true;};
 _return = true;
...
player addAction ["<t color='#C9C900'>Fahrzeug anheben</t>", "core\vehicle\Lifting\lift.sqf", nil, 10, true, true, "", "call CUP_checkLift"];
player addAction ["<t color='#C9C900'>Fahrzeug abwerfen</t>", "core\vehicle\Lifting\drop.sqf", nil, 10, true, true, "", "call CUP_checkDrop"];

player addEventHandler ["Respawn", {player addAction ["<t color='#C9C900'>Fahrzeug anheben</t>", "core\vehicle\Lifting\lift.sqf", nil, 10, true, true, "", "call CUP_checkLift"]; player addAction ["<t color='#C9C900'>Fahrzeug abwerfen</t>", "core\vehicle\Lifting\drop.sqf", nil, 10, true, true, "", "call CUP_checkDrop"];}];
