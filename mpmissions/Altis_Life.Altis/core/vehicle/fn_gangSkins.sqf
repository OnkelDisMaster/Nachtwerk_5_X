    ...
    switch (_lizenzen select _i) do
    {
        case "license_civ_gang_as": 
            {           
                switch (_vehicle) do 
                {
                    case "B_MRAP_01_F": { skinAction = player addAction["[AS] Hunter Skin", '(vehicle player) setObjectTextureGlobal[0,"skins\car\Hunter_asiimov_0.paa"]; (vehicle player) setObjectTextureGlobal[1,"skins\car\Hunter_asiimov_1.paa"]; player removeAction skinAction;', "",0,false,true,"""",'(vehicle player != player) && ((typeOf (vehicle player)) isEqualTo "B_MRAP_01_F")' ]; };
