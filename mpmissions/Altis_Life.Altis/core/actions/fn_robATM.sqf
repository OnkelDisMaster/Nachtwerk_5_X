...
_rip = true;
_atmgeld = 5000 + round(random 8000);
...
if (log_atm_rob >= 4) then { _atmgeld = 50 + round(random 100); 
    [1,format["*** Ein leerer Geldautomat wird aufgebrochen in %1. ***", _shop]]    remoteExecCall ["life_fnc_broadcast",west];
    _chance = 35;  };

if(_chance >= 25) then { hint "Die Polizei wurde benachrichtig!"; [1,format["Ein Geldautomat wird aufgebrochen in %1!", _shop]] remoteExecCall ["life_fnc_broadcast",west]; };
...
titleText[format["Du hast $%1 gestohlen!",[_atmgeld] call life_fnc_numberText],"PLAIN"];
 
life_cash = life_cash + _atmgeld;
