...
diag_log '<infiSTAR.de> VERSION: 04-Dec-2016 13-30-40 infiSTAR.de AHAT (v0245a)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='bernotat-dominik@web.de';
INFISTARVERSION='04-Dec-2016 13-30-40 - v0245a';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    bernotat-dominik@web.de - 04-Dec-2016 13-30-40 - v0245a - %2 - %3',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [bernotat-dominik@web.de - 04-Dec-2016 13-30-40 - v0245a - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    bernotat-dominik@web.de - 04-Dec-2016 13-30-40 - v0245a - %2 - %3',_t,serverName,productVersion];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['X','V','G','8','h','4','w','3','y','v','W','v','A','P','0','L','Z','Y','L','V','5','o','3','I','5','i','x','O','I','b'];
...
_FNC_AH2925_KICKLOG = call _fnc_RandomGen;format['_FNC_AH2925_KICKLOG: %1',_FNC_AH2925_KICKLOG] call FNC_A3_LOG_RAND_VAR;
_FNC_AH2925_KICKLOGSPAWN = call _fnc_RandomGen;format['_FNC_AH2925_KICKLOGSPAWN: %1',_FNC_AH2925_KICKLOGSPAWN] call FNC_A3_LOG_RAND_VAR;
...
_AH2925KICKLOG = call _fnc_RandomGen;format['_AH2925KICKLOG: %1',_AH2925KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
_AH2925_KICKLOG = call _fnc_RandomGen;format['_AH2925_KICKLOG: %1',_AH2925_KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
_fnc_infiSTAR_PlayerLog = {
	...
	if({((toLower _name) find _x != -1)} count ['thirtysix','le hippo'] > 0)exitWith{
		...
		[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
	};
...
fnc_infiSTAR_PlayerLog = compileFinal ([_fnc_infiSTAR_PlayerLog] call fnc_CompilableString);
							...
							"; if(_UVC)then{ _A3AHstring = _A3AHstring + "
								if((!("+str _UVW+") && {_vehstype in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_vehstype in "+str _VehicleWhiteList+")}))then
								{
									...
									if(_crew isEqualTo [])then
									{
										...
										['SERVER','SERVER','HLOG',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
									...
									else
									{
										...
										_log = format['BadVehicle (S-UVC+LVC): %1',_vehstype];
										{
											_name = name _x;
											_puid = getPlayerUID _x;
											[_name,_puid,'HLOG',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
										} forEach _crew;
										_veh call fnc_deleteObject;
									};
								};
						...
						if(!isNull _veh)then
						{
							if((_plrObj == _veh) && (_vehstype != _vehptype))then
							{
								...
								if!(_veh in _vehicles)then
								{
									...
									[_name,_puid,'SLOG',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
								};
							};
						};
					};
	...
	"+_FNC_AH2925_KICKLOG+" = compileFinal ([_FNC_AH2925_KICKLOG] call fnc_CompilableString);
			...
			if(_puid == _clientUID)then
			{
				...
				[
					'',
					{
						...
						"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
							if(isNil 'fnc_adminLog')then{if(_puid in "+str _devs+")then{fnc_adminLog = compileFinal '';}else{fnc_adminLog = compileFinal '[profileName,getPlayerUID player,''ALOG'',toArray _this] call "+_AH2925KICKLOG+";';};};
	...
	_FNC_AH2925_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH2925_KICKLOGSPAWN _this is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		diag_log format['<infiSTAR.de FNC_AH2925_KICKLOGSPAWN> %1',_this];
		...
		diag_log format['<infiSTAR.de FNC_AH2925_KICKLOGSPAWN> %1(%2) Token %3 - objByNetID %4',_name,_puid,_tokenreceived,_objectFromNetId];
		...
		else
		{
			if(_objectUID isEqualTo _puid)then
			{
				...
				{
					diag_log format['<infiSTAR.de FNC_AH2925_KICKLOGSPAWN> %1(%2) PlayerObjectName is not equal to PlayerName!',_name,_puid,_objectName];
					_name = _objectName;
				};
			};
		};
		...
		if(!_result)exitWith
		{
			...
			diag_log format['<infiSTAR.de FNC_AH2925_KICKLOGSPAWN> %1',_log];
			[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
		};
		
		
		_arraysent set [0,_objectName];
		_arraysent set [1,_puid];
		_arraysent spawn "+_FNC_AH2925_KICKLOG+";
	};
	"+_FNC_AH2925_KICKLOGSPAWN+" = compileFinal ([_FNC_AH2925_KICKLOGSPAWN] call fnc_CompilableString);
...
_AH_MAIN_BLOCK = {
	...
	if(isNil'"+_AH2925KICKLOG+"')exitWith{
	...
	if!(_puid isEqualTo _clientUID)exitWith
	{
		...
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH2925KICKLOG+";
		(findDisplay 46)closeDisplay 0;
	};
	...
	if!(_puid in _admins)then
	{
		...
		if(_add_Id > -1)then
		{
			...
			if(_announce)then
			{
				...
				[profileName,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH2925KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		};

		{
			if(!isNil _x)then
			{
				...
				[profileName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH2925KICKLOG+";
				(findDisplay 46)closeDisplay 0;
			};
		} forEach [
		...
		_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
		{
			...
			if(_lconfigName in _badclassesban)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_AH2925KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
			if(_lconfigName in _badclasseskick)then
			{
				_log = format['Bad Addon found in CfgPatches: %1',_configName];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH2925KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		} forEach _cfgPatches;
			...
			_fadeOut = {
				...
				if(!isNull findDisplay 602)then
				{
					...
					if(time > _lastCall)then
					{
						...
						[_name,_puid,'HLOG',toArray(_log)] call "+_AH2925KICKLOG+";
					};
				};
			};
			_dis_head = 0.20;
...
if(_admin)then
{
	if!(_ltxt isEqualTo '''')then
	{
		if(_ltxt isEqualTo ''!admin'')exitWith
		{
			...
			[_name,_puid,''AC'',toArray(_txt)] call "+_AH2925KICKLOG+";
...
"; if(_VDN)then{ _A3AHstring = _A3AHstring + "
	if(_ltxt in [''!vote day'',''/vote day'',''vote day'',''!day'',''/day''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''DAY''] call "+_AH2925KICKLOG+";
	};
	if(_ltxt in [''!vote night'',''/vote night'',''vote night'',''!night'',''/night''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''NIGHT''] call "+_AH2925KICKLOG+";
	};
"; }; _A3AHstring = _A3AHstring + "
false
';
_chatKeyUp_id = -1;
		...
		while{1==1}do
		{
						...
						if(_shape isEqualTo 'POLYLINE')then
						{
							...
							if!(_puid isEqualTo '')then
							{
								...
								if(_puid isEqualTo _varuid)then
								{
									...
									if(_need_hlog)then
									{
										[_name,_varuid,'HLOG',toArray(_log)] call "+_AH2925KICKLOG+";
									};
									_need_hlog = false;
								};
							};
			...
			if(_combatTime <= 30)then
			{
				...
				{
					_inCombat = 1;
					[_name,_puid,'COMBATCHECK',_inCombat] call "+_AH2925KICKLOG+";
				};
			}
			else
			{
...
_chatKeyUp_id = (finddisplay 24) displayAddEventHandler ['KeyUp',_chatKeyUp];
		...
		_log = 'Loop #0 ended!';
		[profileName,getPlayerUID player,'BAN',toArray(_log)] call "+_AH2925KICKLOG+";
		[] spawn "+_AHKickOFF+";
	};
	[_name,_puid,_admins] spawn {
		_name = _this select 0;
									...
									{
										{
											if!((toLower _var) find _x isEqualTo -1)then
											{
												...
												[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call "+_AH2925KICKLOG+";
												[] spawn "+_AHKickOFF+";
												...
												"; if(_UBV2C)then{ _A3AHstring = _A3AHstring + "
													...
													if(!isNil '_m')then
													{
														...
														if((toLower _mstring) find _x != -1)then
														{
															...
															[profileName,getPlayerUID player,'SLOG',toArray(_log)] call "+_AH2925KICKLOG+";
															(findDisplay 46)closeDisplay 0;
															missionNamespace setVariable[_var,nil];
														};
													};
		...
		[_name,_puid] spawn {
			...
			_FiredCheck = {
				...
				if(("+str _UFA+") && {((!("+str _UAW+") && {_ammo in "+str _ForbiddenAmmo+"}) || (("+str _UAW+") && {!(_ammo in "+str _AmmoWhiteList+")}))})then
				{
					...
					[profileName,getPlayerUID _unit,'BAN',toArray(_log)] call "+_AH2925KICKLOG+";
					[] spawn "+_AHKickOFF+";
				};
								...
								if(_maxAmmo > 1)then
								{
									...
									if(diag_tickTime - "+_lastshotmade+" > 0.2)then
									{
										...
										if((_camm >= _maxAmmo)&&(_muzzle isEqualTo currentWeapon _unit))then
										{
											...
											[profileName,getPlayerUID _unit,'SLOG',toArray(_log)] call "+_AH2925KICKLOG+";
				...
				if((!isNull (findDisplay 12))&&(visiblemap))then
				{
					...
					if(!_loggedOnce)then
					{
						...
						if(_size > 5)then
						{
							...
							[_name,_puid,'SLOG',toArray(_log)] call "+_AH2925KICKLOG+";
							_loggedOnce = true;
							lbClear _control2;
						};
					};
						...
						if((isNull _driver)||{(player isEqualTo _driver)})then
						{
							if(!isNil '"+_AHpos+"')then
							{
								...
								if(_tmpAHpos isEqualTo [])then
								{
									...
									if(_tpcount > 3)then
									{
										[_name,_puid,'TMPBAN',toArray(_log)] call "+_AH2925KICKLOG+";
										[] spawn "+_AHKickOFF+";
									}
									else
									{
								...
								else
								{
									...
									[_name,_puid,'TPLOG',toArray(_log),_tmpAHpos select 0,_tmpAHpos select 1,_lastpos,_curpos] call "+_AH2925KICKLOG+";
								};
							}
							else
							{
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
					...
					[] spawn {
						...
						if(time - "+_clickOnMapTimer+" > 5)then
						{
							...
							if("+_clickOnMapCaught+" >= 3)then
							{
								...
								[profileName,getPlayerUID player,''SLOG_SKICK'',toArray(_log)] call "+_AH2925KICKLOG+";
							};
						};
					};
				';
			...
			while{1==1}do
			{
				...
				if(!isNull player)then
				{
					...
					"; if(_LVC)then{ _A3AHstring = _A3AHstring + "
						...
						if(!(player isEqualTo _veh) && !(_vehptype isEqualTo (typeOf _veh)))then
						{
							...
							{
								[_name,_puid,'LVC',player,_vehptype,mapGridPosition _veh] call "+_AH2925KICKLOG+";
							};
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',life_cash,_rnd1])] call "+_AH2925KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo life_adminlevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Adminlevel Hack: [%1] should be [%2]',life_adminlevel,_rnd2])] call "+_AH2925KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo life_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',life_coplevel,_rnd3])] call "+_AH2925KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo life_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_fnc_MPexec [%1] should be [%2]',life_fnc_MPexec,_rnd4])] call "+_AH2925KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo life_atmbank)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_atmbank [%1] should be [%2]',life_atmbank,_rnd5])] call "+_AH2925KICKLOG+";[] spawn "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '2925';
	_devLog = format['<infiSTAR.de> %1 VERSION: 04-Dec-2016 13-30-40 infiSTAR.de AHAT (v0245a) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_vers = '04-Dec-2016 13-30-40 - v0245a';
			...
			{
				{
					...
					{
						_name = name _x;
						[_name,_puid,'BAN',toArray('')] call "+_FNC_AH2925_KICKLOG+";
					};
				} forEach allPlayers;
...
_DO_THIS_MORE_OFTEN = {
	...
	if(hasInterFace)then
	{
		...
		} forEach ['"+_AH2925_KICKLOG+"','"+_adminsA+"','"+_MC+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
		...
		};
		{
			...
			if(!isNil '_pNs')then
			{
				...
				[profileName,getPlayerUID player,'HLOG',toArray(_log)] call "+_AH2925KICKLOG+";
				profileNamespace setVariable[_x,nil];saveprofileNamespace;
			};
		...
		while{1==1}do
		{
			...
			{
				_timer0 = time + 20;
				'"+_AH2925_KICKLOG+"' addPublicVariableEventHandler {(_this select 1) call "+_FNC_AH2925_KICKLOGSPAWN+";};
				uiSleep 1;
											...
											if!(_uid in "+_tokenSentArray+")then
											{
												if(_uid in "+_connectedArray+")then
												{
													...
													if(_timePassed > 60)then
													{
														...
														[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_FNC_AH2925_KICKLOG+";
													};
												};
											};
										"; }; _A3AHstring = _A3AHstring + "
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '
			sleep 0.1;
			(findDisplay 46)closeDisplay 0;
			if(!isNil''"+_AH2925KICKLOG+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AH2925KICKLOG+";};
		';
		if(isNil'"+_AHKickOFF+"')then{"+_AHKickOFF+" = _AHKickOFF;};
		if(str _AHKickOFF != str "+_AHKickOFF+")then{[] spawn _AHKickOFF;};
		_AH2925KICKLOG = compileFinal ""
			if(!isNil'"+_YourPlayerToken+"')then
			{
				"+_AH2925_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH2925_KICKLOG+"';"+_AH2925_KICKLOG+"=nil;
			}
			else
			{
				_this spawn {
					...
					if(isNil'"+_YourPlayerToken+"')exitWith{KICKED_FOR_NIL_TOKEN_AH2925KICKLOG = format['PLAYER__%1__%2',getPlayerUID player,_this];publicVariableServer 'KICKED_FOR_NIL_TOKEN_AH2925KICKLOG';(findDisplay 46)closeDisplay 0;};
					"+_AH2925_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH2925_KICKLOG+"';"+_AH2925_KICKLOG+"=nil;
				};
			};
		"";
		if(isNil'"+_AH2925KICKLOG+"')then{"+_AH2925KICKLOG+" = _AH2925KICKLOG;};
		if(str _AH2925KICKLOG != str "+_AH2925KICKLOG+")then{[] spawn _AHKickOFF;};
