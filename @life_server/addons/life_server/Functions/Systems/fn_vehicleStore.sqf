...
if (_impound) exitWith {
    if (count _vInfo isEqualTo 0) then  {
        ...
        _query = format["UPDATE vehicles SET active='0', fuel='%3', damage='%4', chip='%5' WHERE pid='%1' AND plate='%2'",_uid , _plate, _fuel, _damage, _chip];
...
// update
_query = format["UPDATE vehicles SET active='0', inventory='%3', gear='%4', fuel='%5', damage='%6', chip='%7' WHERE pid='%1' AND plate='%2'", _uid, _plate, _trunk, _cargo, _fuel, _damage, _chip];
_thread = [_query,1] call DB_fnc_asyncCall;
